...
// entry, not the whole cache, so unrelated reads remain warm.
//
...
// Invalidate removes a single key from the cache. Exposed so callers
// outside this package (e.g. an admin operation that pokes settings
