...
SELECT c.id,
       ...
       c.pieces_ids,
       c.target_platforms,
       COALESCE((SELECT ct.id FROM campaign_types ct WHERE ct.name = c.objective), 'Uk'),
       c.status,
       c.start_date,
       c.end_date,
