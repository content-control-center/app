...
// fakeSettings is an in-memory zernio.SettingsStore.
...
func (s *fakeSettings) Get(_ context.Context, k string) (string, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	v, ok := s.kv[k]
	return v, ok, nil
}
func (s *fakeSettings) Set(_ context.Context, k, v string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.kv[k] = v
	return nil
}
func (s *fakeSettings) Delete(_ context.Context, k string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	delete(s.kv, k)
	return nil
...
func seedPublishedPost(repo *fakePostRepo, id, publisherPostID string) {
...
func newRefreshProcessor(stub *stubZernio, postRepo *fakePostRepo, analyticsRepo repository.PostAnalyticsRepository, settings zernio.SettingsStore) *queues.RefreshZernioAnalyticsProcessor {
	return &queues.RefreshZernioAnalyticsProcessor{
		...
		Settings: settings,
		// Every left zero: no self-reschedule in unit tests (no backlite ctx).
	}
}
...
func TestRefreshMatchesAndUpsertsOnlyKnownPosts(t *testing.T) {
	...
	seedPublishedPost(postRepo, "post-1", "z-1")
	...
	if v, _, _ := settings.Get(context.Background(), zernio.SettingAnalyticsLastRefreshStatus); v != zernio.SyncStatusOK {
		t.Errorf("refresh status: got %q want ok", v)
	}
	if v, _, _ := settings.Get(context.Background(), zernio.SettingAnalyticsLastRefreshAt); v == "" {
...
func TestRefreshPagesThroughAllPages(t *testing.T) {
	...
	postRepo := newFakePostRepo()
	seedPublishedPost(postRepo, "post-1", "z-1")
	seedPublishedPost(postRepo, "post-2", "z-2")
	analyticsRepo := newFakeAnalyticsRepo()

	proc := newRefreshProcessor(stub, postRepo, analyticsRepo, newFakeSettings())
	if err := proc.Process(context.Background(), queues.RefreshZernioAnalyticsTask{}); err != nil {
		t.Fatalf("process: %v", err)
	}
...
func TestRefreshLegacy402IsTerminalNoRetry(t *testing.T) {
	...
	postRepo := newFakePostRepo()
	seedPublishedPost(postRepo, "post-1", "z-1")
	proc := newRefreshProcessor(stub, postRepo, newFakeAnalyticsRepo(), newFakeSettings())
